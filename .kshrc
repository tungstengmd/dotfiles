...
trap 'i=0; [[ ${istrans:-u} = u  ]] || t="$(date +%s)"; [[ $istrans = 0 ]] && { tput cuu $((`fc -lnN0 | sed "s/[[:blank:]]//" | wc -l`+1)); tput ed; print -n "\E[92m\E[7m$(v=`pwd | sed "s|^$HOME|~|"`; ((`print $v | wc -m` > 15)) && print -n "~/..." || print -n $v)\E[27m-%\E[0m "; fc -lnN0 | sed "s/^[ \t]*//"; print "\E[A"; istrans=; }; failsafe=0' DEBUG || failsafe=1
