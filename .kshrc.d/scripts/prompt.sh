...
function prompt {
    ...
    if [[ $PS1_MIN = 0 ]]; then
        ...
        print -n "\E[92m╭─{${owo}}─{`date +%H:%M`}`[ $(id -u) = 0 ] && print "\E[91m" || print "\E[93m"` `whoami` \E[92min \E[7m$PWD_TRUNC\E[27m$brnch$symb$e% \E[0m"
    else
        istrans=1
        print -n "`[ $(id -u) = 0 ] && print "\E[91m" || print "\E[93m"``whoami` \E[92min\E[94m $PWD_TRUNC`[ $error = 0 ] || print "\E[91m [$error]"`\E[94m$brnch$symb \E[0m% "
    fi
}
function PS1.get {
    error=$?
